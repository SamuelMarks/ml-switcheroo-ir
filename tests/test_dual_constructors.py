...
def test_logical_node_conflicting_op_type_and_kind() -> None:
    ...
    with pytest.raises(ValueError, match="Conflicting op_type"):
...
def test_logical_graph_explicit_edges() -> None:
    ...
    graph = LogicalGraph(name="EdgeGraph", nodes=[n1, n2], edges=edges)
    assert n2.inputs == ["n1"]
...
def test_logical_graph_outputs_deduction_and_override() -> None:
    ...
    graph_explicit = LogicalGraph(
        name="ExplicitOutputs", nodes=[n1, n2], outputs=["custom_out"]
    )
    ...
    graph_cycle = LogicalGraph(nodes=[c1, c2])
    ...
    graph_multi = LogicalGraph(nodes=[split, c_s0])
